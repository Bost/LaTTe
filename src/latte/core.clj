...
(defn ^:no-doc handle-def-axiom-thm [stmt stmt-name doc params body]
  (when (defenv/registered-definition? stmt-name)
    (println "[Warning] redefinition as" (name stmt) ":" stmt-name))
  (let [[status definition] (handle-de stmt stmt-name params body)]
    (if (= status :ko)
      [:ko definition nil nil nil]
      (let [metadata (cond->>
                         {:doc (mk-def-doc (clojure.string/capitalize (name stmt)) body doc)
